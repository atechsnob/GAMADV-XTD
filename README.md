...
```
admin@server:/usr/local/gamadv-xtd$ ls -l $OLDGAMPATH/*.json
...
admin@server:/usr/local/gamadv-xtd$ 
```
Initialize GAMADV-XTD; this should be the first GAMADV-XTD command executed.
```
admin@server:/usr/local/gamadv-xtd$ ./gam config verify
...
```
admin@server:/usr/local/gamadv-xtd$ ls -l ~/.gam
total 48
...
admin@server:/usr/local/gamadv-xtd$ cp -p $OLDGAMPATH/client_secrets.json ~/.gam/
admin@server:/usr/local/gamadv-xtd$ cp -p $OLDGAMPATH/oauth2service.json ~/.gam/
admin@server:/usr/local/gamadv-xtd$ ls -l ~/.gam
total 40
-rw-r-----+ 1 admin  staff   553 Mar  3 09:23 client_secrets.json
-rw-r-----+ 1 admin  staff  1069 Mar  3 09:23 gam.cfg
...
-rw-r-----+ 1 admin  staff  2377 Mar  3 09:23 oauth2service.json
```
Update your project to include the additional APIs that GAMADV-XTD uses.
```
...
```
Enable GAMADV-XTD client access, create oauth2.txt; it must be deleted and recreated because it is in a different format than in basic Gam.
```
admin@server:/usr/local/gamadv-xtd$ rm -f ~/.gam/oauth2.txt
admin@server:/usr/local/gamadv-xtd$ ./gam version
WARNING: Config File: /Users/admin/.gam/gam.cfg, Section: DEFAULT, Item: oauth2_txt, Value: /Users/admin/.gam/oauth2.txt, Not Found
GAM 4.54.46 - https://github.com/taers232c/GAMADV-XTD
...
admin@server:/usr/local/gamadv-xtd$ ./gam oauth create
...
```
admin@server:/usr/local/gamadv-xtd$ ./gam user testuser1@domain.com check serviceaccount
...
```
admin@server:/usr/local/gamadv-xtd$ ./gam info domain
...
admin@server:/usr/local/gamadv-xtd$ ./gam config customer_id C01234567 domain domain.com save verify
...
This example assumes that GAMADV-XTD has beem installed in C:\GAMADV-XTD.
Set environment variable OLDGAMPATH to point to the existing Gam directory; C:\GAM will be used in this example.
```
C:\GAMADV-XTD>set OLDGAMPATH=C:\GAM
```
Verify that OLDGAMPATH points to the correct location.
```
C:\GAMADV-XTD>dir %OLDGAMPATH%\*.json
...
Initialize GAMADV-XTD; this should be the first GAMADV-XTD command executed.
```
C:\GAMADV-XTD>gam config verify
...
C:\GAMADV-XTD>copy %OLDGAMPATH%\client_secrets.json %HOMEPATH%\.gam\
        1 file(s) copied.

C:\GAMADV-XTD>copy %OLDGAMPATH%\oauth2service.json %HOMEPATH%\.gam\
        1 file(s) copied.
...
Enable GAMADV-XTD client access, create oauth2.txt; it must be deleted and recreated because it is in a different format than in basic Gam.
```
C:\GAMADV-XTD>del %HOMEPATH%\.gam\oauth2.txt
...
Gam service account access must be enabled.
```
C:\GAMADV-XTD>gam user testuser1@domain.com check serviceaccount
User: testuser1@domain.com, Check 12 Scopes
  Scope: https://mail.google.com/                                    , Checked: PASS (1/12)
  Scope: https://sites.google.com/feeds                              , Checked: FAIL (2/12)
  Scope: https://www.google.com/m8/feeds                             , Checked: FAIL (3/12)
...
C:\GAMADV-XTD>gam config customer_id C01234567 domain domain.com save verify
